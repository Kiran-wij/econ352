...
group_tt <- rate_changes %>%
  arrange(diff)  %>%
  head(10)

group_cc <- rate_changes %>%
  arrange(desc(diff))  %>%
  head(10)
...
group_cc <- group_cc %>%
  mutate(rate_01 = tax_rate_merged$topstaterate if year == 2001)
